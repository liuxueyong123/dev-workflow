...
| 留档只保存修订后的极简版本，丢失初稿细节 | 选择留档时保存完整归档包：raw draft、反馈/决策、最终确认版、变更摘要 |
...
- **留档保真**：留档不能只保存被 grill 或反馈压缩后的最终摘要。`spec.md` 需要保留原始 spec 草稿、修改请求/决策、最终确认版和变更摘要；`plan.md` 需要保留原始 plan 草稿、真实 grill Q&A/发现、最终确认版和变更摘要。
